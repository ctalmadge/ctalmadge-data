...
if [ "$DBCHECK" == "$MYTABLE" ]; then
   ...
   mysql -u"$MYSQLUSER" -p"$MYSQLPASS" -e "CREATE TABLE $MYTABLE (ID VARCHAR(255), Date TIMESTAMP, BirthLocation VARCHAR(255), FavoriteAnimal VARCHAR(255), Hobby VARCHAR(255), Age NUMERIC(5,2), DreamJob VARCHAR(255)); ALTER TABLE $MYTABLE ADD PRIMARY KEY (ID);" $MYDATABASE
fi
